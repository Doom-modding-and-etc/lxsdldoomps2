...
extern boolean strafeonrightjoy;        // ptek
...
void I_PollJoystick(void)
{
	...
	if ( hat == SDL_HAT_CENTERED ){
		...
		if (strafeonrightjoy == false)		// ptek
        xaxisl = SDL_JoystickGetAxis(joystick, 0) / 3000;
        else		// ptek
		xaxisl = SDL_JoystickGetAxis(joystick, 2) / 3000;		// ptek
		if (abs(xaxisl) < 3)
			ev.data1 = 0;
		else if ( xaxisl > 0 )
			ev.data1 = 1;
		...
		xaxisr = SDL_JoystickGetAxis(joystick, 0) / 3000;		// ptek
