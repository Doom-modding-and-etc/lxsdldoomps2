...
boolean M_WriteFile(char const* name,void* source,int length)
{
    ...
    handle = fioOpen ( name, O_WRONLY | O_CREAT | O_TRUNC);       // ptek
	if (handle == -1)
		return false;

	//count = write (handle, source, length);
    count = fioWrite (handle, source, length);     // ptek
	//close (handle);
    fioClose (handle);     // ptek
...
int M_ReadFile(char const* name,byte** buffer)
{
	...
	//struct stat fileinfo;     // ptek
	byte   *buf;

	//handle = open (name, O_RDONLY | O_BINARY, 0666);
    handle = fioOpen (name, O_RDONLY);        // ptek
	
	if (handle == -1)
	{
		I_Error ("Couldn't read file %s", name);
		//if (fstat (handle,&fileinfo) == -1) {       // ptek : fstat not implemented in ps2sdk
		if (1)
		{
			//close(handle);
			fioClose(handle);       // ptek
	...
	//length = fileinfo.st_size;        // ptek : fstat not implemented in ps2sdk
    // ptek : getting file size by other mean
    length = fioLseek(handle, 0, SEEK_END);
	fioLseek(handle, 0, SEEK_SET);

	//printf("lenght = %d", length);      // ptek
	buf = Z_Malloc (length, PU_STATIC, NULL);
	//count = read (handle, buf, length);
    count = fioRead (handle, buf, length);     // ptek
...
printf("lenght = %d", length);      // ptek
buf = Z_Malloc (length, PU_STATIC, NULL);
handle = fioOpen ("mc0:DOOM/BOOMSAV0.dsg", O_RDONLY);       
...
//{boom_compatibility},0,boom_compatibility,      /* ptek : testing */
    ...
    {"sfx_volume",{&snd_SfxVolume},{15},0,15, def_int,ss_none},      // ptek
	...
	//{"key_strafeleft",  {&key_strafeleft},     {PS2_L2}           ,	// ptek
	{"key_strafeleft",  {&key_strafeleft},     {-1}           ,	// ptek
	...
	{"key_speed",       {&key_speed},          {PS2_CROSS}    ,		// ptek
	...
	{"key_gamma",       {&key_gamma},          {PS2_R3}       ,     // ptek (was -1)
    ...
    // ptek : added
    {"key_weaponnext",{&key_weaponnext},        {PS2_CIRCLE}            ,
	0,MAX_KEY,def_key,ss_keys}, // key to select next weapon with ammo
    {"key_weaponprevious",{&key_weaponprevious},        {PS2_SQUARE}            ,
	0,MAX_KEY,def_key,ss_keys}, // key to select previous weapon with ammo
...
#define ropen fopen     // ptek : bypass ROMFS
#define rclose fclose
#define rread fread
...
int count = 0;
    ...
    unsigned int test, lndef, lncfg;        // ptek
...
printf("%s\n", defaultfile);
    ...
    // ptek : crap, fseek / ftell don't work with mc0:
    f = fioOpen (defaultfile, O_RDONLY);
    if (f)
    {
        size = fioLseek(f, 0, SEEK_END);
...
//f = fopen("mass:boom.cfg", "r");

	if (f)
	{
printf("FILE OPENED\n");

        //fseek(f,0,SEEK_END);        // ptek
        //size = ftell(f);
        //fseek(f,0,SEEK_SET);

		//while (!feof(f))      // ptek : doesn't seems to work anyway
        //while (ftell(f) < size)       // ptek : ftell doesn't works with mc0 ? crap
        while (count < size)       // ptek
		{
			isstring = false;
            ...
            // ptek : replacement for "if (fscanf (f, "%79s %[^\n]\n", def, strparm) == 2)"

            fgets(boomcfgline, sizeof(boomcfgline), f);           
            //printf("%s\n", boomcfgline);
				...
				if ((defaults[i].type != def_none) && !strcmp(def, defaults[i].name))
				{
					...
					if (!isstring)
					{
						...
						if ((defaults[i].minvalue==UL || defaults[i].minvalue<=parm) &&
                            ...
                            *(defaults[i].location.pi) = parm;		// ptek : parameter is set here
					...
					else
					{
						free((char*)*(defaults[i].location.ppsz));    // phares 4/13/98
                            *(defaults[i].location.ppsz) = newstring;		// ptek : parameter is set here
					}
					break;
				}
			}

            //if (strcmp("weapon_choice_9", def) == 0)      // ptek : ugly hack since feof don't give a shit about the mc0
            //    break;
		}
