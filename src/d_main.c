...
extern boolean strafeonrightjoy = false;        // ptek
...
char *D_DoomExeDir(void)
{
	...
	if (!base)        // cache multiple requests
	{
        ...
        // ptek : only mkdir if "mc0:DOOM/BOOMSAV0.dsg" does not exists
        int handle = fioOpen ("mc0:DOOM/BOOMSAV0.dsg", O_RDONLY);
        if (handle == -1)
            fioMkdir(base); // Make sure it exists
        else
            fioClose(handle);
        
//printf("fioMkdir disabled * * * *  * \n");		// ptek
/*
int handle;
byte   *buf;
buf = Z_Malloc (100, PU_STATIC, NULL);
handle = fioOpen ("mc0:DOOM/BOOMSAV0.dsg", O_RDONLY);       
int count = fioRead (handle, buf, 100);     
...
{
    // ptek : hack : look for special file triggeting a config option
    FILE	       *h1;
	h1 = fopen("host:strafeonrightjoy.dat", "rb");
    if (h1 != NULL)
    {
        printf(">>>>>activating strafeonrightjoy\n");
        strafeonrightjoy = true;
        fclose(h1);
    }
    else
    {
        h1 = fopen("mass:strafeonrightjoy.dat", "rb");
        if (h1 != NULL)
        {
            printf(">>>>>activating strafeonrightjoy\n");
            strafeonrightjoy = true;
        ...
        {
            h1 = fopen("strafeonrightjoy.dat", "rb");
            if (h1 != NULL)
            {
                printf(">>>>>activating strafeonrightjoy\n");
                strafeonrightjoy = true;
