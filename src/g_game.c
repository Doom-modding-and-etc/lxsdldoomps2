...
int     key_weaponnext;       // ptek : added
int     key_weaponprevious;       // ptek : added
...
void G_BuildTiccmd(ticcmd_t* cmd)
{
    ...
    //ptek : next/previous weapon

    if (gamekeydown[key_weaponnext])                                 
    {
        newweapon = P_NextWeapon(&players[consoleplayer]);      //ptek
    }
    
    if (gamekeydown[key_weaponprevious])                                 
    {
        newweapon = P_PreviousWeapon(&players[consoleplayer]);      //ptek
    } 
...
void G_DoLoadGame(void)
{
	...
	if (compatibility_level < lxdoom_1_compatibility) {
		...
		save_p += sizeof(long);
        */
printf("alternative to LONG (little endian)\n");
        // ptek : alternative to LONG (little endian)
        unsigned char bt1, bt2, bt3, bt4;
        bt1 = *save_p++;
        bt2 = *save_p++;
        bt3 = *save_p++;
        bt4 = *save_p++;
        leveltime = (bt4<<24) + (bt3<<16) + (bt2<<8) + bt1;
        bt1 = *save_p++;
        bt2 = *save_p++;
        bt3 = *save_p++;
        bt4 = *save_p++;
        totalleveltimes = (bt4<<24) + (bt3<<16) + (bt2<<8) + bt1;
	}
...
printf("name: %s\n", name);        // ptek
printf("basesavegame: %s, savegamename : %s\n", basesavegame, savegamename);        // ptek
#endif
}
		...
		*save_p++ = idmusnum;               // jff 3/17/98 save idmus state
		if (compatibility_level < lxdoom_1_compatibility) {
			...
			*save_p++ = leveltime;
            ...
            /*
            *(long*)save_p = LONG(leveltime);       // ptek : crash
            save_p += sizeof(long);
			*(long*)save_p = LONG(totalleveltimes);
			save_p += sizeof(long);
            */
printf("alternative to LONG (little endian)\n");
            *save_p++ = (long)leveltime & 255;        // ptek : alternative to LONG (little endian)
            *save_p++ = ((long)leveltime>>8) & 255;
            *save_p++ = ((long)leveltime>>16) & 255;
            *save_p++ = ((long)leveltime>>24) & 255;
...
byte *G_WriteOptions(byte *demo_p)
{
	...
	*demo_p++ =  rngseed        & 0xff;
	...
	if (demo_p != target)
    {
        ...
        printf("G_WriteOptions: GAME_OPTION_SIZE is too small\n");      // ptek
    }
